...
In this project, youll be writing a Python module that uses the PostgreSQL
database to keep track of players and matches in a game tournament.The
game tournament will use the Swiss system (See repo for details) for
pairing up players in each round players are not eliminated, and each player
should be paired with another player with the same number of wins, or as
close as possible. This project has two parts: defining the database
schema (SQL table definitions), and writing the code that will use it.
...
"""
    CONN_STRING = "host='localhost' dbname='tournament2'
...
def playerStandings():
    ...
    sorted by wins.The first entry in the list should be
    the player in first place, or a player tied for first
    place if there is currently a tie.
    ...
    try:
        ...
        cursor.execute("""select
            ...
            on player.id = player_games_played.id join player_games_won
            on player_games_played.id = player_games_won.id order by wins
             DESC; """)
...
def swissPairings():
    ...
    try:
        ...
        cursor.execute("""select player1, p1.name, player2, p2.name from
                         ...
                         b.id as player2,
                         b.wins as wins2
                         from player_games_won as a join player_games_won
                         as b
                         on a.wins = b.wins and a.id < b.id ) as c join
                         player
                         as
                         p1 on c.player1 = p1.id
                         join player as p2 on c.player2 = p2.id
                         order by wins1 desc """)
        records = cursor.fetchall()
        # pprint.pprint(records)
        return records
